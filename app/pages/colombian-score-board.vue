...
<template>
  <UDashboardPanel id="home">
    <template #header>
      <UDashboardNavbar title="Colombian Score Board" :ui="{ right: 'gap-3' }">
        <template #leading>
    ...
    <template #body>
      <div class="space-y-6">
        ...
        <div>
          ...
          <AnalyticsStats :period="period" :range="range" :retention-only="retentionOnly" :lead-vendors="COLOMBIAN_LEAD_VENDORS" />
        </div>

        <!-- Performance Section -->
        <div>
          <h2 class="text-lg font-semibold text-highlighted mb-4">Performance Rates</h2>
          <AnalyticsRates :period="period" :range="range" :retention-only="retentionOnly" :lead-vendors="COLOMBIAN_LEAD_VENDORS" />
        </div>
      </div>
    </template>
  </UDashboardPanel>
