...
const COLOMBIAN_LEAD_VENDORS = ['INB BPO', 'MBG', 'Alternative BPO', 'JoLu Solutions']
const isColombianUser = computed(() => userId.value === RESTRICTED_USER_ID)
...
const loadData = async (page = 0) => {
  ...
  // Apply Colombian vendor filter
  if (isColombianUser.value) {
    filterObj.leadVendors = COLOMBIAN_LEAD_VENDORS
  }
    ...
    cell: ({ row }) => {
      ...
      return h(UButton, {
        ...
        onClick: () => {
          ...
          selectedNotes.value = row.original.notes
          selectedInsuredName.value = row.original.insured_name || 'Unknown'
        }
      })
    }
  },
...
watch(() => insuredNameSearch.value, async () => {
...
<template>
  <UDashboardPanel id="daily-deal-flow">
    <template #header>
      <UDashboardNavbar title="Colombian Deal Flow">
        <template #leading>
    ...
    <template #body>
      ...
      <div class="flex items-center justify-between gap-3 border-t border-default pt-4 mt-auto">
       ...
       <UModal v-model:open="notesModalOpen" :title="'Notes - ' + selectedInsuredName" class="max-w-lg">
         <template #body>
           <p class="whitespace-pre-wrap break-words text-sm">{{ selectedNotes }}</p>
         </template>
       </UModal>
     </template>
   </UDashboardPanel>
